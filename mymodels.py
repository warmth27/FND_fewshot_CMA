...
class Adapter_V1(torch.nn.Module):
    def __init__(self, num_classes=2):
        super(Adapter_V1, self).__init__()   # 调用父类，初始化
        self.fc = nn.Linear(512 * 2, num_classes)   # 全连接层，处理拼接后的特征
        self.fc_txt = nn.Linear(512, num_classes)   # 全连接层，处理文本
        self.fc_img = nn.Linear(512, num_classes)   # 全连接层，处理图像
        self.cross_attention = CrossAttention(512)   # 交叉注意力机制，融合文本和图像特征
        self.fc_meta = nn.Linear(num_classes * 5, num_classes)   # 全连接层，处理融合后的特征
